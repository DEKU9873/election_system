...
import { FinanceCapitalsHeader } from "../../Components/auth/TableHeaderData";
...
const FinanceCapitalsPage = () => {
  ...
  const [financeCapitals, loading] = GetAllFinanceCapitalsHook();
  ...
  const filteredData = useMemo(() => {
    return financeCapitals.filter(
      (item) =>
        item?.title?.toLowerCase().includes(filterText.toLowerCase()) ||
        item?.description?.toLowerCase().includes(filterText.toLowerCase())
    );
  }, [financeCapitals, filterText]);
  ...
  return (
    <div>
      ...
      >
        <div className="mb-6">
          <UserTableTitle title="رئس مال" subtitle="قائمة رؤوس الاموال" />

          <UserTableToolbar
            title="اضافة رئس مال جديد"
          ...
          <UserTableStats data={financeCapitals} title="اجمالي رؤوس الاموال" />
        </div>
        ...
        <div className="border border-gray-200 rounded-lg shadow-sm overflow-x-auto md:overflow-x-visible">
          <table className="w-full min-w-[800px]">
            <UserTableHeader
              tableHeaders={FinanceCapitalsHeader}
              visibleColumns={visibleColumns}
...
export default FinanceCapitalsPage;
